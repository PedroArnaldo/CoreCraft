...
class BitcoinRPC:
    ...
    def call(self, method: str, params=None, wallet=None):
        if params is None:
            params = []

        url = self._url
        if wallet and wallet != self.wallet:
            # Se wallet específica for passada, constrói URL temporária
            url = f"http://{self.host}:{self.port}/wallet/{wallet}"
...
class WalletState:
    ...
    def set(self, wallet_name):
        self.selected = wallet_name
